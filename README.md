...
This project includes code originally developed by SuiteInsider, licensed under the MIT License from [this repo](https://github.com/dsvantien/netsuite-mcp-server) (published to npm as `@suiteinsider/netsuite-mcp`, vendored here at `1.0.2`). Vendored for Qualifyze's supply-chain review: pin by commit instead of `npx @latest` against an unreviewed third-party publisher with OAuth access to production financials. Default `OAUTH_CALLBACK_PORT` changed from `8080` to an ephemeral port (`55182`) to avoid first-run collisions with common local dev servers.
...
2. Fill in the details:
   ...
   - **Redirect URI**: `http://localhost:55182/callback` (or your custom port)
...
- `OAUTH_CALLBACK_PORT` - OAuth callback port (default: 55182)
...
- **OAUTH_CALLBACK_PORT** - OAuth callback port (optional, default: 55182)
...
6. **Callback**: NetSuite redirects to `http://localhost:55182/callback` with authorization code
...
**Solution**:
```bash
# Check what's using the port (example for port 55182)
lsof -i :55182
...
&redirect_uri=http://localhost:55182/callback
&client_id={client_id}
